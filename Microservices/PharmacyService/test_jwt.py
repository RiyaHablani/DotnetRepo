...
def main():
    ...
    print("=== Pharmacy Service API Testing ===")
    print()
    ...
    test_endpoint(f"{base_url}/api/medicines", admin_token)
    test_endpoint(f"{base_url}/api/medicines/unexpired", admin_token)
    test_endpoint(f"{base_url}/api/medicines/disease-type/Diabetes", admin_token)
    test_endpoint(f"{base_url}/api/medicines/search?name=Paracetamol", admin_token)
    
    # Test Prescription endpoints
    print("Testing Prescription endpoints:")
    test_endpoint(f"{base_url}/api/prescriptions", admin_token)
    test_endpoint(f"{base_url}/api/prescriptions/patient/1", admin_token)
    test_endpoint(f"{base_url}/api/prescriptions/status/Pending", admin_token)
    
    # Test with Patient role
    print("\n2. Testing with Patient role:")
    patient_token = create_test_jwt("Patient", patient_id=1)
    print(f"Patient Token: {patient_token[:50]}...")
    print()
    
    # Test patient-specific access
    test_endpoint(f"{base_url}/api/prescriptions/patient/1", patient_token)
    test_endpoint(f"{base_url}/api/prescriptions/patient/2", patient_token)  # Should fail
    
    # Test creating a medicine (should work with Admin)
    print("\n3. Testing Medicine Creation:")
    medicine_data = {
    ...
    test_endpoint(f"{base_url}/api/medicines", admin_token, "POST", medicine_data)
